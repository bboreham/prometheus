...
func newTestClientAndQueueManager(t *testing.T, cfg config.QueueConfig, mcfg config.MetadataConfig, flushDeadline time.Duration) (*TestWriteClient, *QueueManager) {
	...
	m := NewQueueManager(metrics, nil, nil, nil, dir, newEWMARate(ewmaWeight, shardUpdateDuration), cfg, mcfg, labels.EmptyLabels(), nil, c, flushDeadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
	return c, m
}
...
func TestShutdown(t *testing.T) {
	...
	m := NewQueueManager(metrics, nil, nil, nil, dir, newEWMARate(ewmaWeight, shardUpdateDuration), cfg, mcfg, labels.EmptyLabels(), nil, c, deadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
...
func TestReshardRaceWithStop(*testing.T) {
	...
	go func() {
		for {
			metrics := newQueueManagerMetrics(nil, "", "")
			m = NewQueueManager(metrics, nil, nil, nil, "", newEWMARate(ewmaWeight, shardUpdateDuration), cfg, mcfg, labels.EmptyLabels(), nil, c, defaultFlushDeadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
			m.Start()
			h.Unlock()
			h.Lock()
			m.Stop()
...
func TestReshardPartialBatch(t *testing.T) {
	...
	metrics := newQueueManagerMetrics(nil, "", "")
	m := NewQueueManager(metrics, nil, nil, nil, t.TempDir(), newEWMARate(ewmaWeight, shardUpdateDuration), cfg, mcfg, labels.EmptyLabels(), nil, c, flushDeadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
	m.StoreSeries(series, 0)
...
func BenchmarkSampleSend(b *testing.B) {
	...
	metrics := newQueueManagerMetrics(nil, "", "")
	m := NewQueueManager(metrics, nil, nil, nil, dir, newEWMARate(ewmaWeight, shardUpdateDuration), cfg, mcfg, labels.EmptyLabels(), nil, c, defaultFlushDeadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
	m.StoreSeries(series, 0)
...
func BenchmarkStartup(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		m := NewQueueManager(metrics, nil, nil, logger, dir,
			...
			cfg, mcfg, labels.EmptyLabels(), nil, c, 1*time.Minute, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
...
func TestCalculateDesiredShards(t *testing.T) {
	...
	m := NewQueueManager(metrics, nil, nil, nil, dir, samplesIn, cfg, mcfg, labels.EmptyLabels(), nil, c, defaultFlushDeadline, labels.NewSymbolTable(), newHighestTimestampMetric(), nil, false, false)
