...
func processExternalLabels(st *labels.SymbolTable, ls labels.Labels, externalLabels []labels.Label) labels.Labels {
	if len(externalLabels) == 0 {
		return ls
	}

	b := labels.NewScratchBuilder(st, ls.Len()+len(externalLabels))
	j := 0
